...
class ChatUI {
    ...
    getPromptKeys(language, engine) {
        ...
        if (isFeatureChecked && isPageChecked) {
            ...
            if (this.isJavaSelenium(lang, eng)) {
                ...
                // For non-supported combinations, generate feature only and warn
    ...
    addUnsupportedLanguageMessage(language, engine) {
        const message = `⚠️ ${language}/${engine} combination is not yet supported. Supported combinations: Java/Selenium and TypeScript/Playwright.`;
