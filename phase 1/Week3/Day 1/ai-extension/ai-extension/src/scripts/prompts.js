...
export const DEFAULT_PROMPTS = {
  ...
  CUCUMBER_WITH_SELENIUM_JAVA_STEPS: `
    ...
    Output Format:
    - Gherkin in a \`\`\`gherkin\`\`\` block + Java code in a \`\`\`java\`\`\` block.
...
DEFAULT_PROMPTS.PLAYWRIGHT_TS_PAGE_ONLY = `
    Instructions:
    - Generate ONLY a Playwright TypeScript Page Object / Page class (no test runner code).
    - Use Playwright Test (v1.30+) compatible imports and TypeScript types.
    - Add JSDoc / TypeScript doc comments for methods & class.
    - Use meaningful method names.
    - Do NOT include explanations or test runner scaffolding.
...
DEFAULT_PROMPTS.CUCUMBER_WITH_PLAYWRIGHT_TS_STEPS = `
    Instructions:
    - Generate BOTH:
      1. A Cucumber .feature file (Gherkin).
      2. A TypeScript step definition file using Playwright for interactions.
    - Step defs must include Playwright \`Page\` setup, waits, and actual Playwright code.
    - Use Scenario Outline with Examples table when applicable.
    ...
    // ... additional step defs using Playwright APIs
    \`\`\`

    Persona:
    - Audience: QA engineers using Cucumber with Playwright + TypeScript.

    Output Format:
    - Gherkin in a \`\`\`gherkin\`\`\` block + TypeScript step defs in a \`\`\`ts\`\`\` block.
